...
PROJECT_NAME="${CIRCLE_PROJECT_REPONAME}-db.${BUILD_NO}"
ZIP_FILE="${PROJECT_NAME}.zip"

zip -r "${BASE_DIR}/${ZIP_FILE}" "${RELEASE_DIR}/deploy_db.yml"
...
for i in ${!namelist[@]}; do
    if [[ ${namelist[$i]} == "${CIRCLE_PROJECT_REPONAME}-db" ]];
    then
        ...
        status_code=$(curl --silent --output /dev/stderr --write-out "%{http_code}" -X POST ${OCTOPUS_BASE}/api/releases -H "X-Octopus-ApiKey:${OCTO_API_KEY}" -H "content-type:application/json" -d "{\"ProjectId\":\"${project_id}\", \"Version\":\"${BUILD_NO}\", \"ChannelId\":\"${channel_id}\",\"SelectedPackages\": [{\"StepName\": \"Unpack Deployment Assets\",\"ActionName\": \"Unpack Deployment Assets\",\"Version\": \"${BUILD_NO}\"}]}")
